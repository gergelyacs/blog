...
Legyen $Y_{i,j}^t = 1$, ha az $i$-edik előfizető meglátogatta $T_j$ tornyot a $t$ időtartományban, máskülönben 0. **A támadás célja az $\mathbf{Y}^t$ mátrix rekonstruálása minden $t$-re** úgy, hogy (1) $\sum_{i=1}^n Y_{i,j}^t = p_{j}^t$, vagyis az $\mathbf{Y}^t$ mátrix $j$ oszlopában található elemek összege a $T_j$ tornyot meglátogatott előfizetők száma egy időpontban, valamint (2) $\sum_{j=1}^m Y_{i,j}^t = 1$, vagyis egy előfizető egy időpontban (itt órában) csak egy tornyot látogathat meg. Mivel vannak előfizetők akik nem minden órában használják a telefonukat, az ilyen órákban a feltételezett toronylátogatásokat interpolációval becsüljük az ismert toronylátogatásokból.
