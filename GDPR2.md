...
Van amikor a cég megengedi, hogy kérdezők (támadók) specifikálják a lekérdezéseket (*interaktív, online eset*), de van amikor csak  előre meghatározott lekérdezések eredményeit teszik közzé. Az utóbbi esetben a kérdező (támadó) nem tudja a lekérdezéseket megválasztani (*nem interaktív, offline eset*). Nyilván az interaktív eset veszélyesebb adatvédelmi szempontból, hiszen akkor a támadó a lekérdezéseit a korábbi lekérdezések eredményeinek a függvényében állíthatja össze (*adaptív támadás*).
...
Gondolhatnánk, hogy az ilyen támadások megelőzhetők ha nem válaszolunk meg olyan lekérdezéseket, amelyek értéke túl kicsi (vagyis túl kevés rekordra illeszkedik a lekérdezés mintája). Sajnos ez nem mindig segít; pl. az 1. táblázatban az összes Crohn betegek száma 3 (első lekérdezés), a budapesti Crohn betegek száma 2 (második lekérdezés), akkor a kettő különbsége adja a vidéki Crohn betegek számát amire csak egy rekord illeszkedik (K. Ferenc) és amit így már nem kell lekérdezni (*differencing attack*). Az ilyen "problémás" lekérdezések detekciója nem mindig könnyű, általános esetben pedig ez egy [eldönthetetlen probléma](https://en.wikipedia.org/wiki/Undecidable_problem).
...
Vegyük példaként a 2. és 3. táblázatot, ahol 2. táblázat a 3. táblázatban szereplő egyének kvázi azonosítóit tartalmazza. A 3. táblázat
első attribútuma az illető korát tartalmazza (ugyanaz mint a 2. táblázat 3. oszlopa), a többi attribútumok pedig Budapest főbb helyeinek ([POI](https://hu.wikipedia.org/wiki/POI)) felelnek meg, és értékük 1, ha az illető meglátogatta az adott helyet, egyébként nulla. Tegyük fel, hogy a két táblázat rekordjai ugyanahhoz a személyhez tartoznak (vagyis a 3. táblázat $i$-ik sora és 4. táblázat $i$-ik sora ugyanazon személy adatai).  
...
Vagyis $Q(X,Y)$ azon rekordok számát jelöli, ahol a kor értéke legalább $X$ és legfeljebb $Y$, és $Z$ attribútum/hely értéke 1. A támadó célja a 3. táblázat rekonstruálása a lekérdezések értékét és a 2. táblázatot felhasználva. Továbbá tegyük fel, hogy nem válaszolunk meg olyan lekérdezéseket, amelyek eredménye túl kicsi (pl. ha $Q([X,Y],Z) < 5$), az adatbázisban pedig több száz rekord van.
...
[Bebizonyítható](http://www.cse.psu.edu/~ads22/privacy598/papers/dn03.pdf), hogy ha $t= n \log^2 n$ megválaszolt lekérdezés esetén az 6. lépésben kapott $x_i$ értékek nagy része helyes (tehát $x_1, x_2, \ldots, x_n$ megegyezik a 3. táblázat $Z$ értének megfelelő oszlopával), és így az eredeti adatbázis $Z$ értékének megfelelő oszlopa  rekonstruálható. Pl. ha az adatbázisban 128 rekord van, akkor abból legalább 124 helyreállítható így. Más szavakkal a támadás [sikervalószínűsége](http://blog.crysys.hu/2017/07/628/) jelentős.
...
3. A megoldandó egyenletrendszer $n$ változót és $t= n \log^2 n$ egyenletet/feltételt tartalmaz. [Létezik olyan támadás](https://www.microsoft.com/en-us/research/wp-content/uploads/2008/08/dy08.pdf), ami csak $t= n$ lekérdezést (egyenletet) igényel és $O(n\log n)$ futási idővel hasonló rekonstrukcós pontosságot ér el, viszont itt a lekérdezéseket determinisztikusan és nem véletlenszerűen választják. 
4. Ezek a támadások akkor is működhetnek, ha a [lekérdezések értékéhez véletlen zajt adunk hozzá](http://www.cse.psu.edu/~ads22/privacy598/papers/dn03.pdf) (query perturbation). Ebben az esetben egyenletek helyett egyenlőtlenségeket kell megoldani,  aminek  megoldása az első támadással az elméletileg *legrosszabb esetben* $O(n^5 \log^4 n)$ lépést igényel, míg a másodikkal csak $O(n\log n)$. Ha a zaj által okozott hiba legfeljebb $E$, akkor $n - 4E^2$ rekord [helyreállítható]((https://www.microsoft.com/en-us/research/wp-content/uploads/2008/08/dy08.pdf)), ahol $n$ a rekordok száma az adatbázisban. Pl. ha $n=256$ és $E=3$, akkor 220 rekord helyreállítható. Viszont érdekes módon $E > \sqrt{n}$ torzítás már megakadályozza a rekordok visszaállítását.
...
Tegyük fel, hogy a $(t-1)$-beli toronylátogatások ismertek ($\mathbf{Y}^{t-1}$), valamint az összes torony látogatottsága $t$-ben ($\mathbf{P}^t$). Hogyan tudnánk ezekből rekonstruálni $\mathbf{Y}^{t}$-t?
...
Lényegében az alábbi optimalizációs problémát kell megoldanunk minden $t$ időpontban, ahol az ismeretlen $\mathbf{Y}^{t}$:
